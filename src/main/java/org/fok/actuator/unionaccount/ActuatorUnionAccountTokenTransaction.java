...
import org.fok.core.model.Transaction.*;
import org.fok.tools.bytes.BytesHashMap;
...
public class ActuatorUnionAccountTokenTransaction extends AbstractTransactionActuator {
	public ActuatorUnionAccountTokenTransaction(IAccountHandler iAccountHandler,
			ITransactionHandler iTransactionHandler, Block.BlockInfo oBlock, ICryptoHandler iCryptoHandler) {
	...
	public void onPrepareExecute(TransactionInfo transactionInfo, BytesHashMap<AccountInfo.Builder> accounts)
		...
		unionAccountBalance = this.iAccountHandler.getTokenBalance(unionAccount, oInput.getToken().toByteArray());
	...
	public ByteString onExecute(TransactionInfo transactionInfo, BytesHashMap<AccountInfo.Builder> accounts)
		...
		TransactionInput oInput = transactionInfo.getBody().getInputs();
		AccountInfo.Builder unionAccount = accounts.get(oInput.getAddress().toByteArray());
		this.iAccountHandler.setNonce(unionAccount, oInput.getNonce() + 1);
		...
		this.iAccountHandler.subTokenBalance(unionAccount, oInput.getToken().toByteArray(),
				BytesHelper.bytesToBigInteger(oInput.getAmount().toByteArray()));

		accounts.put(unionAccount.getAddress().toByteArray(), unionAccount);
